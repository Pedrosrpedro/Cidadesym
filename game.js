const Game = {
    ...
    initializeLogicalGrid: function() { /* ...código inalterado... */ },

    // ATUALIZADO: setupScene agora cria o terreno 3D e a água
    setupScene: function() {
        ...
        // NOVO: Cria o terreno da ilha e a água
        this.terrainMesh = this.createIslandTerrain();
        this.terrainMesh.userData.isGround = true;
        this.scene.add(this.terrainMesh);

        this.waterMesh = new THREE.Mesh(
            new THREE.PlaneGeometry(this.gridWorldSize * 2, this.gridWorldSize * 2, 10, 10),
            new THREE.MeshLambertMaterial({
                color: 0x006994,
                transparent: true,
                opacity: 0.7,
                // NOVO: Adiciona textura de água se existir
        ...
        this.waterMesh.position.y = 1.0; // Nível da água
        ...
        // NOVO: Cursor para pré-visualizar a terraplanagem
    ...
    // NOVO: Função para gerar o terreno da ilha
    createIslandTerrain: function() {
        ...
        for (let i = 0; i < vertices.count; i++) {
            ...
            const z = vertices.getY(i); // Em PlaneGeometry, Y é o nosso Z
            const dist = center.distanceTo(new THREE.Vector2(x, z));

            // Fórmula para criar uma forma de ilha com queda suave (radial falloff)
            const falloff = Math.pow(1 - (dist / maxDist), 2.5);
            let height = maxHeight * falloff;
            
            // Adiciona um pouco de ruído simples para não ser perfeitamente liso
            height += (Math.sin(x * 0.1) * Math.cos(z * 0.1) * 2.0);
            
            // Garante que a borda seja um pouco abaixo do nível da água
            height = Math.max(height, 0);
            
            vertices.setZ(i, height); // Em PlaneGeometry, Z é a altura
        }
        geometry.computeVertexNormals(); // Recalcula as normais para a iluminação correta

        const grassTex = new THREE.TextureLoader().load('assets/grass.png');
        grassTex.wrapS = grassTex.wrapT = THREE.RepeatWrapping;
        grassTex.repeat.set(50, 50);

        const material = new THREE.MeshLambertMaterial({ map: grassTex, color: 0x55902A });
        const mesh = new THREE.Mesh(geometry, material);
        mesh.rotation.x = -Math.PI / 2;
        return mesh;
    },
    
    // ATUALIZADO: O cursor agora segue a altura do terreno
    updateCursor: function(x, y) {
        ...
        if (intersects.length > 0) {
            ...
            // Lógica de visualização da terraplanagem
            ...
            // Lógica de cor do cursor (inalterada)
    ...
    // ATUALIZADO: A criação de estradas agora usa a altura do terreno
    createRoadSegment: function(start, end) {
        ...
        // NOVO: Terraplana o terreno sob a nova estrada
        this.terraformArea(roadMesh, [startPos, endPos]);

        this.checkForAndProcessIntersections(roadMesh);
    },

    // ATUALIZADO: A criação de curvas também usa a altura do terreno
    finalizeCurvedRoad: function() {
        ...
        // Pega a altura do terreno para cada ponto da curva
    ...
    // ATUALIZADO: A construção de objetos agora terraplana a área
    placeObject: function() {
        ...
        // NOVO: Terraplanagem antes de colocar o objeto
        ...
        newObject.position.copy(position).y = newHeight + (height / 2); // Coloca o objeto na nova altura do terreno
    ...
    // NOVO: Função para terraplanar uma área do terreno
    terraformArea: function(pathObject, pathPoints, areaSize = this.gridSize) {
        ...
        // Calcula a altura média do caminho/área para ser a nova altura plana
        let averageHeight = pathPoints.reduce((sum, p) => sum + p.y, 0) / pathPoints.length;
        
        const modifiedVertices = new Set();
        
        if (pathObject && pathObject.userData.type?.startsWith('road')) {
            // Para estradas, terraplana ao longo do caminho
             const curve = new THREE.CatmullRomCurve3(pathPoints);
             const points = curve.getPoints(Math.floor(curve.getLength()));
             points.forEach(pointOnCurve => {
                 this.findAndModifyVertices(pointOnCurve, this.gridSize, pointOnCurve.y, vertices, modifiedVertices);
             });
        } else {
             // Para edifícios, terraplana uma área quadrada
    ...
    // NOVO: Função auxiliar que encontra e modifica os vértices do terreno em uma área
    findAndModifyVertices: function(center, size, newHeight, vertices, modifiedSet) {
        const halfSize = size / 2;
        for (let i = 0; i < vertices.count; i++) {
            if (modifiedSet.has(i)) continue; // Evita modificar o mesmo vértice duas vezes
            
            const vX = vertices.getX(i);
            const vZ = vertices.getY(i); // Coordenada Z no plano
            
            if (vX >= center.x - halfSize && vX <= center.x + halfSize &&
                vZ >= center.z - halfSize && vZ <= center.z + halfSize) {
                
                vertices.setZ(i, newHeight); // Lembre-se, Z é a altura na geometria do plano
                modifiedSet.add(i);
            }
        }
    },

    // NOVO: Função para obter a altura do terreno em qualquer ponto (x, z)
    ...
    // CORRIGIDO: A função de overlay de energia agora desenha para estradas e cruzamentos
    updatePowerGrid: function() {
        ...
        // LÓGICA DE DESENHO DO OVERLAY CORRIGIDA
        poweredQueue.forEach(poweredObj => {
            if (poweredObj.userData.powerRadius) {
                ...
                } else if (poweredObj.userData.type?.startsWith('road') || poweredObj.userData.type === 'intersection' || poweredObj.userData.type?.startsWith('power')) {
                    // Adiciona um único círculo no centro para estradas, cruzamentos e usinas
                ...
                overlayPositions.forEach(pos => {
                    ...
                    // Pega a altura do terreno para posicionar o overlay corretamente
    ...
    // ATUALIZADO: O nó de cruzamento agora é criado na altura correta
    ...
    // Funções restantes (inalteradas, mas incluídas para garantir a completude)
    ...
    demolishObject: function() { const intersects = this.raycaster.intersectObjects(this.cityObjects, true); if (intersects.length > 0) { let objectToDemolish = intersects[0].object; while (objectToDemolish.parent && objectToDemolish.parent !== this.scene) { objectToDemolish = objectToDemolish.parent; } if (objectToDemolish.userData.type === 'intersection') { const intersectionPoint = objectToDemolish.position; const objectsToRemove = [objectToDemolish]; this.cityObjects.filter(obj => obj.userData.type?.startsWith('road')).forEach(road => { const start = road.userData.startPoint; const end = road.userData.endPoint; if (start?.distanceTo(intersectionPoint) < 0.1 || end?.distanceTo(intersectionPoint) < 0.1) { objectsToRemove.push(road); } }); objectsToRemove.forEach(obj => this.removeObject(obj, false)); this.recalculateAllRoadAdjacency(); this.updatePowerGrid(); } else { this.removeObject(objectToDemolish, true); } } },
    ...
    createPowerLineObject: function(start, end, firstPole) { const startHeight = this.getTerrainHeight(start.x, start.z); const endHeight = this.getTerrainHeight(end.x, end.z); const poleHeight = 12; firstPole.position.y = startHeight + poleHeight / 2; const path = new THREE.Vector3().subVectors(end, start); const length = path.length(); const powerLineGroup = new THREE.Group(); const poleMat = new THREE.MeshLambertMaterial({ color: 0x654321 }); const crossarmGeo = new THREE.BoxGeometry(4, 0.4, 0.4); powerLineGroup.add(firstPole); const secondPole = firstPole.clone(); secondPole.position.copy(end).y = endHeight + poleHeight / 2; powerLineGroup.add(secondPole); const crossarm1 = new THREE.Mesh(crossarmGeo, poleMat); crossarm1.position.copy(start).y = startHeight + poleHeight - 1.5; crossarm1.rotation.y = Math.atan2(path.x, path.z) + Math.PI / 2; powerLineGroup.add(crossarm1); const crossarm2 = crossarm1.clone(); crossarm2.position.copy(end).y = endHeight + poleHeight - 1.5; powerLineGroup.add(crossarm2); const wireGeo = new THREE.BoxGeometry(0.2, 0.2, length); const wireMat = new THREE.MeshLambertMaterial({ color: 0x303030 }); const wire = new THREE.Mesh(wireGeo, wireMat); const midPoint = start.clone().add(path.clone().multiplyScalar(0.5)); const midHeight = (startHeight + endHeight) / 2; wire.position.set(midPoint.x, midHeight + poleHeight - 1.7, midPoint.z); wire.lookAt(end.x, endHeight + poleHeight - 1.7, end.z); powerLineGroup.add(wire); powerLineGroup.userData = { isPowered: false, type: 'connector', powerRadius: this.gridSize * 2.5, consumption: 0.2 }; this.scene.add(powerLineGroup); this.cityObjects.push(powerLineGroup); this.powerConnectors.push(powerLineGroup); this.updatePowerGrid(); },
