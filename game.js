...
const Game = {
    ...
    // NOVO: Sistemas de População e Veículos
    population: 0,
    vehicles: [],
    lastSpawnTime: 0,
    lastTime: 0,
    ...
    init: function() {
        ...
        this.updatePopulationUI(); // Chama a nova função da UI
        this.updatePowerUI();
    },
    ...
    setupScene: function() {
        ...
        this.scene.fog = new THREE.Fog(0x87CEEB, 250, 550);
        ...
        // Posição de câmera melhorada para ver a conexão continental
        ...
        this.terrainMesh = this.createPennisulaTerrain();
        this.terrainMesh.userData.isGround = true; this.scene.add(this.terrainMesh);

        this.waterMesh = new THREE.Mesh( new THREE.PlaneGeometry(this.gridWorldSize * 2, this.gridWorldSize * 2), new THREE.MeshLambertMaterial({ color: 0x006994, transparent: true, opacity: 0.7, map: new THREE.TextureLoader().load('assets/water.png', (texture) => { texture.wrapS = texture.wrapT = THREE.RepeatWrapping; texture.repeat.set(20, 20); }) }) );
        this.waterMesh.rotation.x = -Math.PI / 2; this.waterMesh.position.y = 5.5; this.scene.add(this.waterMesh);
        ...
        this.buildCursor = new THREE.Mesh(cursorGeo, cursorMat); this.buildCursor.visible = false; this.scene.add(this.buildCursor);
        ...
        this.terraformCursor = new THREE.Mesh(terraformGeo, terraformMat); this.terraformCursor.rotation.x = -Math.PI / 2; this.terraformCursor.visible = false; this.scene.add(this.terraformCursor);

        this.powerOverlay = new THREE.Group(); this.powerOverlay.visible = false; this.scene.add(this.powerOverlay);
    },

    // FUNÇÃO DE TERRENO MELHORADA PARA CONEXÃO ÓBVIA E ÁREAS PLANAS
    createPennisulaTerrain: function() {
        ...
        const colors = [];
        const sandColor = new THREE.Color(0xC2B280), grassColor = new THREE.Color(0x55902A), rockColor = new THREE.Color(0x808080), snowColor = new THREE.Color(0xFFFAFA);
        const center = new THREE.Vector2(0, 0); const maxDist = this.gridWorldSize / 2;
        const baseHeight = 18.0; const maxAmplitude = 45.0;

        for (let i = 0; i < vertices.count; i++) {
            const x = vertices.getX(i); const z = vertices.getY(i);
            let noise = 0; let frequency = 1.8 / this.gridWorldSize; let amplitude = maxAmplitude;
            for (let j = 0; j < 4; j++) { noise += Noise.perlin2(x * frequency, z * frequency) * amplitude; frequency *= 2.1; amplitude /= 2.2; }
            const dist = center.distanceTo(new THREE.Vector2(x, z));
            let islandFalloff = Math.pow(1.0 - THREE.MathUtils.smoothstep(dist, maxDist * 0.7, maxDist), 1.2);
            let continentFalloff = Math.pow(THREE.MathUtils.smoothstep(z, maxDist * 0.5, maxDist), 2);
            let finalFalloff = THREE.MathUtils.lerp(islandFalloff, 1.0, continentFalloff);
            const flatAreaFactor = 1.0 - Math.pow(THREE.MathUtils.smoothstep(dist, 70, 150), 2);
            noise *= flatAreaFactor;
            let height = (baseHeight + noise) * finalFalloff;
            if (height < 6) height = 6;
            vertices.setZ(i, height);
            if (height < 10) colors.push(sandColor.r, sandColor.g, sandColor.b); else if (height < 30) colors.push(grassColor.r, grassColor.g, grassColor.b); else if (height < 45) colors.push(rockColor.r, rockColor.g, rockColor.b); else colors.push(snowColor.r, snowColor.g, snowColor.b);
        }
    ...
    createInitialHighway: function() {
        ...
        const startZ = halfSize - 1; const endZ = 0;
        const startY = this.getTerrainHeight(15, startZ) + 0.2;
        const endY = this.getTerrainHeight(15, endZ) + 0.2;
        // Via de Entrada
        this.createRoadObject([new THREE.Vector3(15, startY, startZ), new THREE.Vector3(15, endY, endZ)]);
        // Via de Saída
        this.createRoadObject([new THREE.Vector3(-15, endY, endZ), new THREE.Vector3(-15, startY, startZ)]);
    },
...
setupControls: function() {
    ...
    // CORREÇÃO DEFINITIVA: LÓGICA DE ESTRADAS SIMPLIFICADA E ROBUSTA
    createRoadObject: function(points) {
        ...
        mesh.scale.y = 0.05; // Achatamento do tubo
    ...
    // ATUALIZADO: PLACE OBJECT COM LÓGICA DE OCUPAÇÃO
    placeObject: function() {
        const position = this.buildCursor.position.clone();
        const logicalCoords = this.worldToGridCoords(position);
        if (this.buildMode === 'residential' || this.buildMode === 'commercial') { if (!logicalCoords || !this.logicalGrid[logicalCoords.x] || this.logicalGrid[logicalCoords.x][logicalCoords.z] !== 2) return; }
        ...
        switch (this.buildMode) {
            ...
            case 'commercial': height = this.gridSize * 1.5; objectData.originalColor = new THREE.Color(0x4285F4); newObject = new THREE.Mesh(new THREE.BoxGeometry(this.gridSize, height, this.gridSize), new THREE.MeshLambertMaterial({ color: objectData.originalColor })); objectData.type = 'consumer'; objectData.consumption = 10; this.powerConsumers.push(newObject); break;
        ...
        newObject.position.copy(position).y = newHeight + (height / 2); newObject.userData = objectData; this.scene.add(newObject); this.cityObjects.push(newObject); this.updatePowerGrid();
    },
    
    // ... (funções de terraplanagem, demolição, etc. sem grandes alterações)
    ...
    // NOVAS FUNÇÕES: VEÍCULOS E POPULAÇÃO
    spawnVehicle: function() {
        const unoccupiedHouses = this.powerConsumers.filter(c => c.userData.type === 'consumer' && !c.userData.isOccupied);
        const commercialBuildings = this.cityObjects.filter(c => c.userData.type === 'commercial');
        if (unoccupiedHouses.length === 0 || commercialBuildings.length === 0) return;
        const entryRoad = this.cityObjects.find(o => o.userData.type === 'road' && o.userData.curve.points[0].z > this.gridWorldSize / 2 - 5);
        if (!entryRoad) return;
        ...
        this.vehicles.push({ mesh: car, path: entryRoad.userData.curve, progress: 0, speed: 0.0005 + Math.random() * 0.0008 });
        this.scene.add(car);
    },

    updateVehicles: function(deltaTime) {
        for (let i = this.vehicles.length - 1; i >= 0; i--) {
            const v = this.vehicles[i]; v.progress += v.speed * deltaTime * 100;
            if (v.progress >= 1) { this.scene.remove(v.mesh); this.vehicles.splice(i, 1); this.handleArrival(); continue; }
            const pos = v.path.getPointAt(v.progress); v.mesh.position.copy(pos).y += 1.2;
            const tangent = v.path.getTangentAt(v.progress); v.mesh.lookAt(pos.clone().add(tangent));
        }
    },

    handleArrival: function() {
        const house = this.powerConsumers.find(c => c.userData.type === 'consumer' && !c.userData.isOccupied);
        if (house) {
            house.userData.isOccupied = true;
            house.material.color.set(house.userData.originalColor.clone().multiplyScalar(0.6)); // Escurece a casa
            this.population++;
    ...
    animate: function(time) {
        ...
        const deltaTime = (time - this.lastTime) / 1000; this.lastTime = time;
        if (!this.isInitialized) return; // Garante que a lógica só rode após init completo

        const { x, z } = this.moveDirection;
        if (x !== 0 || z !== 0) {
            if (this.cameraMode === 'move') { this.cameraPivot.translateX(x * this.moveSpeed); this.cameraPivot.translateZ(z * this.moveSpeed); } 
            else { this.cameraPivot.rotateY(-x * this.rotateSpeed); const newRotX = this.camera.rotation.x - z * this.rotateSpeed; if (newRotX > -1.2 && newRotX < 1.2) this.camera.rotation.x = newRotX; }
        }
        ...
        if (time - this.lastSpawnTime > 4000) { // Tenta criar um carro a cada 4 segundos
            if(Math.random() < 0.5) this.spawnVehicle(); // 50% de chance de tentar criar
