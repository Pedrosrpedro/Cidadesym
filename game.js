...
const Game = {
    ...
    temporaryPole: null, currentCurvePoints: [], curveGuideMeshes: [],
    cityObjects: [], powerProducers: [], powerConsumers: [], powerConnectors: [], powerOverlay: null,
    ...
    setupControls: function() {
        ...
        this.joystick = nipplejs.create(options); this.joystick.on('move', (evt, data) => { const angle = data.angle.radian; const force = data.force; this.moveDirection.x = Math.cos(angle) * force; this.moveDirection.z = -Math.sin(angle) * force; }).on('end', () => { this.moveDirection.x = 0; this.moveDirection.z = 0; });
        ...
        document.getElementById('water-overlay-btn')?.addEventListener('click', this.toggleWaterOverlay.bind(this));
        const canvas = this.renderer.domElement; canvas.addEventListener('mousemove', this.updateCursor.bind(this)); canvas.addEventListener('click', this.handleMapClick.bind(this));
    ...
    handleLinePlacement: function() {
        ...
        if (this.buildMode === 'road') {
            ...
            this.currentCurvePoints.push(currentPos); const guideGeo = new THREE.SphereGeometry(1, 8, 8); const guideMat = new THREE.MeshBasicMaterial({ color: 0xffff00 }); const guideMesh = new THREE.Mesh(guideGeo, guideMat); guideMesh.position.copy(currentPos); this.scene.add(guideMesh); this.curveGuideMeshes.push(guideMesh);
        } else if (this.buildMode === 'pipe') {
             if (!this.isDrawing) { this.startPoint = currentPos; this.isDrawing = true; } 
             ...
             if (!this.isDrawing) {
                this.startPoint = currentPos; this.isDrawing = true;
                const poleHeight = 12; const poleGeo = new THREE.CylinderGeometry(0.4, 0.6, poleHeight, 8); const poleMat = new THREE.MeshLambertMaterial({ color: 0x966F33 }); this.temporaryPole = new THREE.Mesh(poleGeo, poleMat); this.temporaryPole.position.copy(this.startPoint).y = this.getTerrainHeight(this.startPoint.x, this.startPoint.z) + poleHeight / 2; this.scene.add(this.temporaryPole);
            } else {
                if (this.temporaryPole) this.scene.remove(this.temporaryPole);
                if (this.startPoint.distanceTo(currentPos) > 0) { this.createPowerLineObject(this.startPoint, currentPos); }
                this.isDrawing = false; this.startPoint = null; this.temporaryPole = null;
            }
        }
    },
    ...
    createRoadSegment: function(start, end) { const startY = this.getTerrainHeight(start.x, start.z) + 0.3; const endY = this.getTerrainHeight(end.x, end.z) + 0.3; this.createRoadObject([new THREE.Vector3(start.x, startY, start.z), new THREE.Vector3(end.x, endY, end.z)]); },
    finalizeCurvedRoad: function() { if (this.currentCurvePoints.length < 2) { this.cancelCurvedRoad(); return; } const pointsWithHeight = this.currentCurvePoints.map(p => new THREE.Vector3(p.x, this.getTerrainHeight(p.x, p.z) + 0.3, p.z)); this.createRoadObject(pointsWithHeight); this.cancelCurvedRoad(); },
    
    createPowerLineObject: function(start, end) {
        const startHeight = this.getTerrainHeight(start.x, start.z); const endHeight = this.getTerrainHeight(end.x, end.z); const poleHeight = 12;
        const powerLineGroup = new THREE.Group(); const poleMat = new THREE.MeshLambertMaterial({ color: 0x654321 }); const poleGeo = new THREE.CylinderGeometry(0.4, 0.6, poleHeight, 8); const crossarmGeo = new THREE.BoxGeometry(4, 0.4, 0.4);
        const firstPole = new THREE.Mesh(poleGeo, poleMat); firstPole.position.copy(start).y = startHeight + poleHeight / 2; powerLineGroup.add(firstPole);
        const secondPole = firstPole.clone(); secondPole.position.copy(end).y = endHeight + poleHeight / 2; powerLineGroup.add(secondPole);
        const path = new THREE.Vector3().subVectors(end, start);
        const crossarm1 = new THREE.Mesh(crossarmGeo, poleMat); crossarm1.position.copy(start).y = startHeight + poleHeight - 1.5; crossarm1.rotation.y = Math.atan2(path.x, path.z) + Math.PI / 2; powerLineGroup.add(crossarm1);
        const crossarm2 = crossarm1.clone(); crossarm2.position.copy(end).y = endHeight + poleHeight - 1.5; powerLineGroup.add(crossarm2);
        const wireGeo = new THREE.BoxGeometry(0.2, 0.2, path.length()); const wireMat = new THREE.MeshLambertMaterial({ color: 0x303030 }); const wire = new THREE.Mesh(wireGeo, wireMat);
        const midPoint = new THREE.Vector3().addVectors(start, end).multiplyScalar(0.5); const midHeight = (startHeight + endHeight) / 2; wire.position.set(midPoint.x, midHeight + poleHeight - 1.7, midPoint.z); wire.lookAt(new THREE.Vector3(end.x, endHeight + poleHeight - 1.7, end.z)); powerLineGroup.add(wire);
    ...
    placeObject: function() { const position = this.buildCursor.position.clone(); const logicalCoords = this.worldToGridCoords(position); if ((this.buildMode === 'residential' || this.buildMode === 'commercial') && (!logicalCoords || this.logicalGrid[logicalCoords.x]?.[logicalCoords.z] !== 2)) return; const objectSize = (this.buildMode === 'power-coal' || this.buildMode === 'sewage-plant') ? this.gridSize * 2 : this.gridSize; const newHeight = this.terraformArea(null, [position], objectSize); let newObject, height = 0; let objectData = { isPowered: false, hasWater: false, hasSewage: false, originalColor: new THREE.Color(0xffffff) }; switch (this.buildMode) { case 'residential': height = this.gridSize; objectData.originalColor.set(0x34A853); newObject = new THREE.Mesh(new THREE.BoxGeometry(this.gridSize, height, this.gridSize), new THREE.MeshLambertMaterial({ color: objectData.originalColor })); objectData.type = 'consumer'; objectData.consumption = 5; objectData.isOccupied = false; this.powerConsumers.push(newObject); break; case 'commercial': height = this.gridSize * 1.5; objectData.originalColor.set(0x4285F4); newObject = new THREE.Mesh(new THREE.BoxGeometry(this.gridSize, height, this.gridSize), new THREE.MeshLambertMaterial({ color: objectData.originalColor })); objectData.type = 'consumer'; objectData.consumption = 10; this.powerConsumers.push(newObject); break; case 'power-wind': height = this.gridSize * 2.5; newObject = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, height, 8), new THREE.MeshLambertMaterial({ color: 0xeeeeee })); objectData.type = 'producer'; objectData.production = 20; objectData.powerRadius = 50; this.powerProducers.push(newObject); break; case 'power-coal': height = this.gridSize * 1.2; newObject = new THREE.Mesh(new THREE.BoxGeometry(this.gridSize*2, height, this.gridSize*1.5), new THREE.MeshLambertMaterial({ color: 0x555555 })); objectData.type = 'producer'; objectData.production = 100; objectData.powerRadius = 80; this.powerProducers.push(newObject); break; case 'water-pump': height = this.gridSize * 0.8; newObject = new THREE.Mesh(new THREE.CylinderGeometry(this.gridSize/2, this.gridSize/2, height, 16), new THREE.MeshLambertMaterial({ color: 0x6fa8dc })); objectData.type = 'water-producer'; objectData.connectionRadius = this.gridSize * 2.5; this.waterProducers.push(newObject); break; case 'sewage-plant': height = this.gridSize; newObject = new THREE.Mesh(new THREE.BoxGeometry(this.gridSize*2, height, this.gridSize*2), new THREE.MeshLambertMaterial({ color: 0x83532c })); objectData.type = 'sewage-processor'; objectData.connectionRadius = this.gridSize * 2.5; this.sewageProcessors.push(newObject); break; default: return; } newObject.position.copy(position).y = newHeight + (height / 2); newObject.userData = objectData; this.scene.add(newObject); this.cityObjects.push(newObject); this.updatePowerGrid(); this.updateWaterGrid(); },
    terraformArea: function(pathObject, buildingPoints = null, areaSize = this.gridSize) { const terrainGeo = this.terrainMesh.geometry; const vertices = terrainGeo.attributes.position; let needsUpdate = false; if (pathObject && pathObject.userData.type?.startsWith('road')) { const pathPoints = pathObject.userData.points; if (!pathPoints || pathPoints.length < 2) return; const modifiedVertices = new Set(); for (const pointOnCurve of pathPoints) { this.findAndModifyVertices(pointOnCurve, areaSize, pointOnCurve.y - 0.3, vertices, modifiedVertices); } needsUpdate = true; } else if (buildingPoints) { const centerPoint = buildingPoints[0]; let totalHeight = 0, count = 0; const halfSize = areaSize / 2; for (let i = 0; i < vertices.count; i++) { const vX = vertices.getX(i), vZ = vertices.getY(i); if (vX >= centerPoint.x - halfSize && vZ <= centerPoint.x + halfSize && vZ >= centerPoint.z - halfSize && vZ <= centerPoint.z + halfSize) { totalHeight += vertices.getZ(i); count++; } } const averageHeight = count > 0 ? totalHeight / count : this.getTerrainHeight(centerPoint.x, centerPoint.z); this.findAndModifyVertices(centerPoint, areaSize, averageHeight, vertices, new Set()); needsUpdate = true; if(needsUpdate) { vertices.needsUpdate = true; terrainGeo.computeVertexNormals(); this.updateTerrainColors(); } return averageHeight; } if(needsUpdate) { vertices.needsUpdate = true; terrainGeo.computeVertexNormals(); this.updateTerrainColors(); } },
    ...
    updatePowerGrid: function() {
        ...
        while (head < poweredQueue.length) {
            ...
            const sourcePositions = (currentPowered.userData.type === 'connector' && currentPowered.isGroup)
                ? currentPowered.children.filter(c => c.geometry.type === 'CylinderGeometry').map(p => p.position)
                : [currentPowered.position];
            
            allPowerObjects.forEach(otherObj => {
                if (otherObj.userData.isPowered) return;
                
                let isClose = false;
                
                const targetPositions = (otherObj.userData.type === 'connector' && otherObj.isGroup)
                    ? otherObj.children.filter(c => c.geometry.type === 'CylinderGeometry').map(p => p.position)
                    : [otherObj.position];

                for (const sourcePos of sourcePositions) {
                    for (const targetPos of targetPositions) {
                        if (sourcePos.distanceTo(targetPos) < radius) {
                            isClose = true;
                            break;
                        }
                    }
                ...
                if (!isClose && otherObj.userData.type === 'road' && otherObj.userData.points) {
                    for (const sourcePos of sourcePositions) {
                        for (const roadPoint of otherObj.userData.points) {
                            if (sourcePos.distanceTo(roadPoint) < radius) {
                                isClose = true;
                                break;
                            }
                        }
        ...
        poweredQueue.forEach(poweredObj => {
            if (poweredObj.userData.powerRadius) {
                const circleGeo = new THREE.CircleGeometry(poweredObj.userData.powerRadius, 32);
                const circleMat = new THREE.MeshBasicMaterial({ color: 0x00aaff, transparent: true, opacity: 0.2 });
                const circleMesh = new THREE.Mesh(circleGeo, circleMat);
                circleMesh.position.copy(poweredObj.position).y = this.getTerrainHeight(poweredObj.position.x, poweredObj.position.z) + 0.15;
        ...
        this.powerConsumers.forEach(c => {
            ...
            c.userData.isPowered = shouldBePowered;
        });
        this.updatePowerUI();
    },
    ...
    togglePowerOverlay: function() { this.powerOverlay.visible = !this.powerOverlay.visible; this.waterOverlay.visible = false; },
